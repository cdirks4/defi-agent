...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!systemPrompt || !userMessage) {
      console.error("Missing parameters:", { systemPrompt: !!systemPrompt, userMessage: !!userMessage });
    ...
    try {
      JSON.parse(response);
      console.log("Successfully parsed AI response in", Date.now() - startTime, "ms");
    } catch (error) {
